...
@section('title', 'Sister Inventory')
...
<style>
    ...
    /* Empty state */
    .empty-state { text-align: center; padding: 40px 25px; color: var(--muted); font-size: 0.88rem; }
...
<div class="page-header">
    <div>
        <h2 class="page-header-title">
            <i class="fa fa-boxes-stacked"></i> Sister Inventory
        </h2>
        <p class="page-header-sub">Manage sister inventory, conditions, and location history</p>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('admin.inventory.export') }}" class="btn-header ghost">
...
<!-- Search Form -->
<form method="GET" action="{{ route('admin.inventory') }}" class="mb-4">
    <div class="d-flex gap-2" style="max-width: 500px;">
        <div style="position: relative; flex: 1;">
            <input type="text" name="search" id="searchInput"
                   class="form-control"
                   placeholder="Search item name, property no., serial no., item set..."
...
<div class="dash-card">
    <div class="dash-card-header">
        <i class="fa fa-table-list"></i> Sister Inventory
    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <form action="{{ route('admin.inventory.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Delete this item?');" style="display:inline">
                                            @csrf @method('DELETE')
            ...
            <form action="{{ route('admin.inventory.update', $item->id) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="serialnum" value="{{ $item->serialnum }}" class="form-control">
                    </div>
                    <div class="mb-3">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.querySelectorAll("select[id^='condition_']").forEach(select => {
            ...
            const origNotes   = uInput ? uInput.value : '';

            const toggle = () => {
                const isUsable = select.value === "Usable";
                wrapper.classList.toggle("d-none", !isUsable);
                if (uInput) uInput.required = isUsable;
                if (fdInput) fdInput.required = isUsable && uInput && uInput.value !== origNotes;
                if (!isUsable) { if (uInput) uInput.value = ""; if (fdInput) fdInput.value = ""; }
                else if (fdInput && fdInput.required && !fdInput.value) { fdInput.value = new Date().toISOString().split('T')[0]; }
            };

            if (uInput) {
                uInput.addEventListener("input", () => {
                    const isUsable = select.value === "Usable";
                    if (fdInput) fdInput.required = isUsable && uInput.value !== origNotes;
                    if (fdInput && fdInput.required && !fdInput.value) fdInput.value = new Date().toISOString().split('T')[0];
                });
            }
